...
while True:
    try:
        with mic as source:
            ...
            # Read audio data from the microphone and continuously transcribe speech
            stream = audioio.I2SIn(bit_clock=BCLK, word_clock=LRCLK, data=DIN, mode=audioio.I2S.PHILIPS, sample_rate=SAMPLE_RATE, data_format=audioio.I2S.MONO)
            buffer_size = 512
            while True:
                buffer = bytearray(buffer_size)
                num_bytes_read = stream.readinto(buffer)
                if num_bytes_read <= 0:
                    break
                audio_data = bytes(buffer)
                try:
                    # recognize speech using Google Speech Recognition
                    text = r.recognize_google_cloud(audio_data, credentials_json=creds)
